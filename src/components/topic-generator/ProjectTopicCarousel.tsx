...
interface ProjectTopicCarouselProps {
  ...
  onGenerate: (rowId: number, inputs: {
    subject: string;
    concept: string;
    topicType: string;
  }) => void;
...
const semesterLabels = ["1학년 1학기 프로젝트", "1학년 2학기 프로젝트", "2학년 1학기 프로젝트", "2학년 2학기 프로젝트", "3학년 1학기 프로젝트"];
const ProjectTopicCarousel: React.FC<ProjectTopicCarouselProps> = ({
  ...
  const handleGenerateTopicsWithWebhook = async () => {
    ...
    try {
      const response = await fetch('https://songssam.demodev.io/webhook/subject-all', {
        method: 'POST',
        headers: {
      ...
      try {
        ...
        const n8nResponseLog = {
          timestamp,
          response: data,
          url: 'https://songssam.demodev.io/webhook/subject-all',
          careerSentence: selectedCareerSentence
        };
        localStorage.setItem('n8n_response_log', JSON.stringify(n8nResponseLog));
        console.log('✅ N8N 응답이 localStorage에 저장되었습니다:', 'n8n_response_log');
      } catch (storageError) {
      ...
      // 모든 가능한 구조 확인
      console.log('data.학기별_프로젝트_가이드라인:', data?.학기별_프로젝트_가이드라인);
      ...
      // 간소화된 N8N 응답 파싱 함수
      const parseN8NResponseSimplified = (responseData: any): DetailedProjectInfo[] | null => {
        ...
        try {
          ...
          if (Array.isArray(responseData)) {
            ...
            if (responseData.length === 5) {
              return responseData.map((item, index) => ({
                주제명: extractString(item, ['프로젝트_주제명', '주제', 'title', 'topic']) || `${index + 1}학기 주제`,
                사전_조사: extractString(item, ['사전_조사', 'research', 'background']) || '',
                핵심_활동: extractString(item, ['핵심_활동', 'activity', 'core_activity']) || '',
                연관_교과목: extractArray(item, ['연관_교과목_및_개념', '연관_교과목', 'subjects', 'related_subjects']) || [],
                사용_도구: extractArray(item, ['배울_툴_프로그램', '사용_도구', 'tools', 'programs']) || []
              }));
            }
          }
          ...
          if (responseData && typeof responseData === 'object') {
            ...
            const guidanceData = responseData.학기별_프로젝트_가이드라인 || 
                               responseData.result?.학기별_프로젝트_가이드라인 ||
                               responseData.response?.학기별_프로젝트_가이드라인;
                               
            if (guidanceData) {
              console.log('✅ 학기별 가이드라인 구조 발견');
              const semesterKeys = ["1학년 1학기", "1학년 2학기", "2학년 1학기", "2학년 2학기", "3학년 1학기"];
              
              return semesterKeys.map((semester, index) => {
                const semesterData = guidanceData[semester];
                if (!semesterData) return null;
                
                return {
                  주제명: extractString(semesterData, ['프로젝트_주제명', '주제', 'title']) || `${semester} 주제`,
                  사전_조사: extractString(semesterData, ['사전_조사', '내용_가이드라인.사전_조사', '내용_가이드라인', 'content.사전_조사']) || '',
                  핵심_활동: extractString(semesterData, ['핵심_활동', '내용_가이드라인.핵심_활동', '내용_가이드라인', 'content.핵심_활동']) || '',
                  연관_교과목: extractArray(semesterData, ['연관_교과목_및_개념', '연관_교과목', 'subjects', 'related_subjects']) || [],
                  사용_도구: extractArray(semesterData, ['배울_툴_프로그램', '사용_도구', 'tools', 'programs']) || []
                };
              }).filter(Boolean);
            }
          }
        ...
        for (const key of keys) {
          if (key.includes('.')) {
            ...
            if (value && typeof value === 'object') {
              const nestedString = extractString(value, ['사전_조사', '핵심_활동', 'content', 'description', 'text']);
      ...
      // 최소 데이터 추출 함수 (마지막 폴백)
      const extractMinimumData = (responseData: any): DetailedProjectInfo[] | null => {
        console.log('=== 최소 데이터 추출 시도 ===');
        
        try {
          const extracted: DetailedProjectInfo[] = [];
          const semesterLabels = ["1학년 1학기", "1학년 2학기", "2학년 1학기", "2학년 2학기", "3학년 1학기"];
          
          // 어떤 형태든 문자열이나 객체에서 최소한의 주제명 추출
          const recursiveExtractTopics = (obj: any, depth: number = 0): string[] => {
          ...
          for (let i = 0; i < 5; i++) {
            extracted.push({
              주제명: foundTopics[i] || `${semesterLabels[i]} 프로젝트 (N8N 응답에서 추출)`,
              사전_조사: '',
              핵심_활동: '',
              연관_교과목: [],
              사용_도구: []
      ...
      if (detailedProjects && detailedProjects.length > 0) {
        ...
        const simpleProjects: DetailedProjectInfo[] = data.topics.map((topic: string) => ({
          주제명: topic,
          사전_조사: '',
          핵심_활동: '',
          연관_교과목: [],
        ...
        // 기존 subject 배열 구조 대응
        const simpleProjects: DetailedProjectInfo[] = data.subject.map((topic: string) => ({
          주제명: topic,
          사전_조사: '',
          핵심_활동: '',
          연관_교과목: [],
        ...
        try {
          ...
          const searchForTopics = (obj: any, path: string = ''): string[] => {
            ...
            if (typeof obj === 'string' && obj.length > 10 && obj.includes('프로젝트')) {
              found.push(obj);
            } else if (Array.isArray(obj)) {
              ...
              Object.keys(obj).forEach(key => {
                if ((key.includes('주제') || key.includes('topic') || key.includes('title')) && typeof obj[key] === 'string') {
                  found.push(obj[key]);
                }
          ...
          if (fallbackTopics.length > 0) {
            const fallbackProjects: DetailedProjectInfo[] = fallbackTopics.slice(0, 5).map((topic, index) => ({
              주제명: topic,
              사전_조사: '',
              핵심_활동: '',
              연관_교과목: [],
            ...
            onUpdateTopicsFromWebhook && onUpdateTopicsFromWebhook(fallbackProjects);
        ...
        if (minimumProjects && minimumProjects.length > 0) {
          ...
          setRegenerateError('N8N에서 예상과 다른 형태의 데이터를 받았습니다. 임시 데이터를 표시합니다.');
          onRegenerateAllTopics();
        }
      }
      ...
      if (error instanceof Error && error.name === 'AbortError') {
        setRegenerateError('AI 주제 생성에 예상보다 오랜 시간이 걸리고 있습니다. 잠시 후 다시 시도해주세요. (AI 생성은 보통 10-20분 정도 소요됩니다)');
      } else if (error instanceof TypeError && error.message.includes('fetch')) {
        setRegenerateError('네트워크 연결에 실패했습니다. 인터넷 연결을 확인하고 다시 시도해주세요.');
      } else {
        setRegenerateError('주제 생성 중 문제가 발생했습니다. 잠시 후 다시 시도해주세요.');
      }
    } finally {
  ...
  const getLoadingMessage = () => {
    if (timeElapsed < 60) {
      ...
      return `AI가 고품질 주제를 생성 중입니다... (${formatTime(timeElapsed)} 경과)`;
    } else if (timeElapsed < 600) {
      return `AI가 심도있는 분석을 진행 중입니다... (${formatTime(timeElapsed)} 경과)`;
    } else if (timeElapsed < 1200) {
      return `AI가 상세한 가이드라인을 작성 중입니다... (${formatTime(timeElapsed)} 경과)`;
    } else {
      return `AI가 최적의 주제를 완성하고 있습니다... (${formatTime(timeElapsed)} 경과)`;
    }
  };
    ...
    try {
      ...
      if (storedLog) {
        ...
        if (typeof response === 'object' && response !== null) {
          ...
          // 학기별 구조 찾기
          const possibleSemesterKeys = ['1학년 1학기', '1학년 2학기', '2학년 1학기', '2학년 2학기', '3학년 1학기'];
          const foundSemesterData = possibleSemesterKeys.find(key => 
            response[key] || response.학기별_프로젝트_가이드라인?.[key]
          );
  ...
  return <div className="w-full max-w-6xl mx-auto">
      ...
      {selectedCareerSentence && !hasGeneratedTopics() && (
        <div className="flex flex-col items-center mb-8">
          ...
          >
            {isRegenerating ? getLoadingMessage() : '전체 학기 프로젝트 주제 생성'}
          </button>
          {isRegenerating && (
      ...
      <Carousel className="w-full max-w-4xl mx-auto" setApi={setApi}>
        <CarouselContent>
          {group.topicRows.slice(0, 5).map((row: any, index: number) => <CarouselItem key={row.id}>
              <div className="border rounded-lg p-6 bg-white shadow-sm h-full">
                ...
                <div className="min-h-[400px] flex flex-col">
                  {/* 단계 1: 초기 상태 */}
                  {row.stage === "initial" && <div className="text-center text-gray-500 py-12">
                  ...
                  {/* 단계 2: 주제 생성 완료 */}
                  {row.stage === "topic_selected" && <div className="flex flex-col space-y-6">
