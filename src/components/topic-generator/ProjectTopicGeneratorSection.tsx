...
interface ProjectTopicGeneratorSectionProps {
  ...
  handleUpdateTopicsFromWebhook: (
    detailedProjects: DetailedProjectInfo[]
  ) => void;
  selectedCareerSentence?: string | null;
  setSelectedCareerSentence: (sentence: string) => void;
}

const ProjectTopicGeneratorSection: React.FC<
  ProjectTopicGeneratorSectionProps
> = ({
  carouselGroups,
  handleAddRow,
  ...
  const handleCareerSentenceGenerate = async (data: {
    ...
    if (
      ...
      (!data.aspiration || !data.aspiration.trim())
    ) {
      console.error("❌ 추가 입력 필드가 비어있습니다 (프로젝트).");
      setGeneratedCareerSentences([
        "직업을 가진 후 하고 싶은 것을 구체적으로 입력해주세요.",
      ]);
      setIsGeneratingCareerSentence(false);
      return;
    }
    ...
    try {
      ...
      const response = await fetch(
        "https://songssam.demodev.io/webhook/request?path=dream",
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        if (resultText) {
          ...
          setGeneratedCareerSentences([
            "텍스트를 추출할 수 없습니다. N8N 응답을 확인해주세요.",
          ]);
        }
      } else {
        console.error(
