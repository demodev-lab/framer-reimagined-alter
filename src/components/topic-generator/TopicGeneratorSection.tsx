...
const TopicGeneratorSection: React.FC<TopicGeneratorSectionProps> = ({
  ...
  return <section className="flex flex-col items-center pb-8">
        ...
        <div className="-mx-[182px]">
          <div className="py-0 flex flex-col gap-8">
            ...
            {topicRows.map((row, index) => <div key={row.id} className="grid grid-cols-1 md:grid-cols-2 gap-8 h-[400px]">
              ...
              }} showFollowUp={index > 0} isFollowUp={followUpStates[row.id] || false} onFollowUpChange={checked => handleFollowUpChange(row.id, checked as boolean)} rowId={row.id} selectedCareerSentence={selectedCareerSentence} />}
                </div>
                <div className="h-full overflow-hidden">
